...
class ErrorMessageTest extends  AnyFlatSpec {
  "give_error" should "work for mismatched closing delimiter" in {
    ...
    val res = ErrorMessage.give_error(fileName,description_error,what_exp,name_of_error,0,codeLines.length,codeLines,underline,
  ...
  "give_error" should "work for expected type" in {
    ...
    val res = ErrorMessage.give_error(fileName,descritpion_error,what_exp,name_of_error,column,column+1,codeLines,underline,column,
